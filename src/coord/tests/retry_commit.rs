...
async fn test_commit_retry_transient_failure() -> anyhow::Result<()> {
    ...
    if let Ok(resp) = fail_resp {
        ...
        println!(
            "Warning: Commit fault injection endpoint not available, assuming retry logic exists"
        );
    }
    ...
    assert_eq!(
        ...
        "PUT should eventually succeed after commit retry"
    );
    ...
    assert!(
        ...
        "Should take some time due to retry"
    );
    ...
    assert_eq!(
        meta.state,
        TxState::Committed,
        "Meta should be committed after retry"
    );
    ...
    assert_eq!(
        ...
        3,
        "All 3 volumes should have the file after commit retry"
    );
    ...
    assert_eq!(
        ...
        "Should be able to read data after commit retry"
    );
...
async fn test_commit_retry_multiple_failures() -> anyhow::Result<()> {
    ...
    assert_eq!(
        status,
        reqwest::StatusCode::CREATED,
        "PUT should succeed despite multiple commit failures"
    );
    assert_eq!(etag, expected_etag, "ETag should match");

    println!(
        "PUT with multiple commit retries completed in {:?}",
        elapsed
    );
    ...
    assert_eq!(
        response_bytes, payload,
        "Data should be intact after multiple commit retries"
    );
...
async fn test_commit_retry_head_node_failure() -> anyhow::Result<()> {
    ...
    assert_eq!(
        status,
        reqwest::StatusCode::CREATED,
        "PUT should succeed after head commit retry"
    );
    assert_eq!(etag, expected_etag, "ETag should match");
    ...
    assert_eq!(
        response_bytes, payload,
        "Data should be intact after head commit retry"
    );
...
async fn test_commit_timeout_then_success() -> anyhow::Result<()> {
    ...
    if let Some(follower_volume) = volumes.iter().find(|v| v.state.node_id == *follower_node) {
        ...
        let _ = client.post(&fail_url).send().await;
        println!(
            "Injected slow/timeout commit on follower: {}",
            follower_node
        );
    }
    ...
    assert_eq!(
        status,
        reqwest::StatusCode::CREATED,
        "PUT should succeed after commit timeout retry"
    );
    assert_eq!(etag, expected_etag, "ETag should match");
