...
async fn test_deterministic_placement() -> anyhow::Result<()> {
    ...
    assert!(
        stayed_keys > 0,
        "All keys moved, should have some stability"
    );
    assert!(
        changed_keys < test_keys.len(),
        "All keys moved, expected minority"
    );

    println!(
        "Placement results: {} keys moved, {} keys stayed",
        changed_keys, stayed_keys
    );
...
async fn test_placement_affinity_with_actual_puts() -> anyhow::Result<()> {
    ...
    for key in &test_keys {
        ...
        assert!(
            expected_node.is_some(),
            "Expected placement for key {}",
            key
        );
