...
async fn test_large_object_streaming_path() -> anyhow::Result<()> {
    ...
    println!("Generating {}MB payload for large object test", payload_size / (1024*1024));
    ...
    println!("Starting PUT with {}MB payload", payload.len() / (1024*1024));
    ...
    // Assert: PUT success
    assert_eq!(status, reqwest::StatusCode::CREATED, "Large object PUT should succeed");
    ...
    assert_eq!(volumes_with_file.len(), 3, "All 3 volumes should have the large file");
    ...
    assert_eq!(meta.size, expected_len, "Meta size should match large object");
    ...
    assert_eq!(response_bytes.len(), payload.len(), "GET response should have correct length");
    
    // For performance, we'll verify hash instead of byte-by-byte comparison
    let response_etag = blake3_hex(&response_bytes);
    assert_eq!(response_etag, expected_etag, "GET response should have correct hash");
...
async fn test_very_large_object_streaming() -> anyhow::Result<()> {
    ...
    println!("Generating {}MB payload for very large object test", payload_size / (1024*1024));
    ...
    assert_eq!(status, reqwest::StatusCode::CREATED, "Very large PUT should succeed");
    ...
    let volume_refs: Vec<&TestVolume> = volumes.iter().collect();
    let volumes_with_file = which_volume_has_file(&volume_refs, key)?;
    assert_eq!(volumes_with_file.len(), 3, "All volumes should have the very large file");
    ...
    assert_eq!(response_bytes.len(), payload.len(), "GET should return correct size");
    
    // Verify data integrity via hash (more efficient than full comparison)
    let response_etag = blake3_hex(&response_bytes);
    assert_eq!(response_etag, expected_etag, "GET data should match original");
...
async fn test_large_object_with_node_failure() -> anyhow::Result<()> {
    ...
    assert!(status.is_server_error(), "Large PUT should fail when node goes down");
    println!("Large PUT correctly failed with node down: {}", status);

    // Verify no files remain
    let volume_refs: Vec<&TestVolume> = volumes.iter().collect();
    let volumes_with_file = which_volume_has_file(&volume_refs, key)?;
    assert_eq!(volumes_with_file.len(), 0, "No remaining volume should have file after failure");
...
async fn test_large_object_concurrent_operations() -> anyhow::Result<()> {
    ...
    println!("All concurrent large uploads completed in {:?}", total_elapsed);
...
async fn test_large_object_memory_efficiency() -> anyhow::Result<()> {
    ...
    println!("Testing memory efficiency with {}MB object", payload.len() / (1024*1024));
    ...
    assert_eq!(status, reqwest::StatusCode::CREATED, "Memory efficiency PUT should succeed");
    assert_eq!(etag, expected_etag, "ETag should match");
    ...
    let response_etag = blake3_hex(&response_bytes);
    assert_eq!(response_etag, expected_etag, "GET should return correct data");

    println!("Memory efficient GET completed in {:?}", get_elapsed);
    println!("Memory efficiency test successful - no OOM with {}MB object", payload.len() / (1024*1024));
