...
async fn test_write_once_semantics() -> anyhow::Result<()> {
    ...
    let (status1, etag1, len1) =
        put_via_coordinator(&client, coord.url(), "same-key", payload.clone()).await?;
...
async fn test_write_once_with_pending_state() -> anyhow::Result<()> {
    ...
    let meta_key = key_utils::meta_key_for("pending%2Dkey");
    ...
    let (status, _etag, _len) =
        put_via_coordinator(&client, coord.url(), "pending-key", payload).await?;
