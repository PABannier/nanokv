...
use coord::command::gc::{gc, GcArgs};
...
async fn test_gc_extraneous_and_orphans_report_only() -> anyhow::Result<()> {
    ...
    let (status, _etag, _size) = put_via_coordinator(&client, coord.url(), key1, content1.to_vec()).await?;
    assert_eq!(status, reqwest::StatusCode::CREATED);
    
    let key2 = "test-key-expected-2";
    let content2 = b"test content expected 2";
    let (status, _etag, _size) = put_via_coordinator(&client, coord.url(), key2, content2.to_vec()).await?;
    assert_eq!(status, reqwest::StatusCode::CREATED);
