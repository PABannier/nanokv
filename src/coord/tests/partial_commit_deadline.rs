...
async fn test_partial_commit_deadline_exceeded() -> anyhow::Result<()> {
    ...
    assert!(
        ...
        "PUT should return 5xx after commit deadline, got: {}",
        status
    );
    println!(
    ...
    assert!(
        elapsed.as_secs() >= 3,
        "Should take at least a few seconds to hit deadline"
    );
    assert!(
        elapsed.as_secs() < 30,
        "Should not take too long (test timeout)"
    );
    ...
    println!(
        "Volumes with file after partial commit: {:?}",
        volumes_with_file
    );
    assert!(
        volumes_with_file.len() < 3,
        "Not all volumes should have the file (some commits failed)"
    );
    ...
    if let Some(meta) = meta {
        assert_ne!(
            meta.state,
            TxState::Committed,
            "Meta should not be Committed since W=N failed"
        );
    ...
    assert!(
        ...
        "GET should fail because meta is not committed, got: {}",
        get_status
    );
...
async fn test_partial_commit_head_succeeds_follower_fails() -> anyhow::Result<()> {
    ...
    if let Some(failing_volume) = volumes
        .iter()
        .find(|v| v.state.node_id == *failing_follower)
    {
        let fail_url = format!("{}/admin/fail/commit?count=5", failing_volume.url());
        let _ = client.post(&fail_url).send().await;
        println!(
            "Injected persistent commit failure on follower: {}",
            failing_follower
        );
    }
    ...
    assert!(
        status.is_server_error(),
        "PUT should fail when any replica fails commit"
    );
    ...
    println!(
        "Volumes with file after partial commit: {:?} (expected < 3)",
        volumes_with_file
    );
    assert!(
        volumes_with_file.len() < 3,
        "Should have partial commit state"
    );
    ...
    if let Some(meta) = meta {
        assert_ne!(
            meta.state,
            TxState::Committed,
            "Meta should not be committed with partial success"
        );
    }
    ...
    assert!(
        !get_status.is_success(),
        "GET should fail with partial commit"
    );
...
async fn test_commit_deadline_cleanup_behavior() -> anyhow::Result<()> {
    ...
    let volumes_with_file = which_volume_has_file(&volume_refs, key)?;
    println!(
        "Volumes with file after cleanup wait: {:?}",
        volumes_with_file
    );
    ...
    if let Some(meta) = meta {
        ...
        if meta.state == TxState::Committed {
            ...
            assert_eq!(
                final_volumes_with_file.len(),
                3,
                "If committed, all volumes should have file"
            );
        }
    }
