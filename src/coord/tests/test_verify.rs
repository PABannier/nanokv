...
mod common;
use common::*;
use ::common::file_utils::{sanitize_key, meta_key_for};

use coord::command::verify::{verify, VerifyArgs};
...
async fn test_verify_under_replication() -> anyhow::Result<()> {
    ...
    let (status, _etag, _size) = put_via_coordinator(&client, coord.url(), key, content.to_vec()).await?;
...
async fn test_verify_corrupted_size_mismatch() -> anyhow::Result<()> {
    ...
    let (status, _etag, size) = put_via_coordinator(&client, coord.url(), key, content.to_vec()).await?;
    assert_eq!(status, reqwest::StatusCode::CREATED);
...
async fn test_verify_unindexed_and_should_gc() -> anyhow::Result<()> {
    ...
    let (status, _etag, _size) = put_via_coordinator(&client, coord.url(), key_normal, content_normal.to_vec()).await?;
    assert_eq!(status, reqwest::StatusCode::CREATED);
    ...
    let (status, _etag, _size) = put_via_coordinator(&client, coord.url(), key_tombstone, content_tombstone.to_vec()).await?;
    assert_eq!(status, reqwest::StatusCode::CREATED);
...
async fn test_verify_per_node_concurrency_respected() -> anyhow::Result<()> {
    ...
    for i in 0..5 {
        ...
        let (status, _etag, _size) = put_via_coordinator(&client, coord.url(), &key, content).await?;
        assert_eq!(status, reqwest::StatusCode::CREATED);
    }
