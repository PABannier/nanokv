...
async fn test_fault_injection_demo() -> anyhow::Result<()> {
    ...
    wait_until(5000, || async {
        let nodes = list_nodes(&coord.state.http_client, coord.url())
            .await
            .unwrap_or_default();
        Ok(nodes.len() >= 3
            && nodes
                .iter()
                .all(|n| matches!(n.status, coord::core::node::NodeStatus::Alive)))
    })
    .await?;
    ...
    let (status, _etag, _len) =
        put_via_coordinator(client, coord.url(), "normal-key", data).await?;
    ...
    let fail_resp = client
        .post(format!(
            "{}/admin/fail/prepare?once=true",
            vol1.state.internal_url
        ))
        .send()
        .await?;
    ...
    let fail_resp = client
        .post(format!(
            "{}/admin/fail/pull?once=true",
            vol2.state.internal_url
        ))
        .send()
        .await?;
    ...
    let fail_resp = client
        .post(format!(
            "{}/admin/fail/commit?once=true",
            vol3.state.internal_url
        ))
        .send()
        .await?;
    ...
    let latency_resp = client
        .post(format!(
            "{}/admin/inject/latency?latency_ms=1000",
            vol1.state.internal_url
        ))
        .send()
        .await?;
    ...
    if latency_resp.status().is_success() {
        ...
        match result {
            Ok((status, _, _)) => {
                println!(
                    "PUT succeeded with injected latency: {} (took {}ms)",
                    status,
                    duration.as_millis()
                );
