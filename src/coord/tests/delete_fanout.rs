...
async fn test_delete_fanout_idempotent() -> anyhow::Result<()> {
    ...
    assert_eq!(
        meta_before.state,
        TxState::Committed,
        "Meta should be committed before delete"
    );
    ...
    let meta_after_delete1 =
        meta_of(&coord.state.db, key)?.expect("Meta should still exist after delete");
    assert_eq!(
        meta_after_delete1.state,
        TxState::Tombstoned,
        "Meta should be tombstoned after first delete"
    );
    ...
    if !volumes_with_file_after_delete1.is_empty() {
        println!(
            "Warning: {} volumes still have file after delete (may be async cleanup)",
    ...
    let meta_after_delete2 =
        meta_of(&coord.state.db, key)?.expect("Meta should still exist after second delete");
    assert_eq!(
        meta_after_delete2.state,
        TxState::Tombstoned,
        "Meta should remain tombstoned after second delete"
    );
    ...
    assert_eq!(
        ...
        "Re-PUT should return 409 due to write-once rule"
    );
...
async fn test_delete_with_some_replicas_down() -> anyhow::Result<()> {
    ...
    assert_eq!(
        volumes_with_file_initial.len(),
        3,
        "All volumes should have file initially"
    );
    ...
    assert_eq!(
        delete_status,
        reqwest::StatusCode::NO_CONTENT,
        "DELETE should succeed with one replica down"
    );
...
async fn test_delete_multiple_keys_fanout() -> anyhow::Result<()> {
    ...
    for key in &keys {
        ...
        assert_eq!(
            meta.state,
            TxState::Tombstoned,
            "Meta should be tombstoned for key: {}",
            key
        );
    }
    ...
    for key in &keys {
        ...
        .unwrap_or_else(|_| {
            println!(
                "Warning: Files for key {} may not be immediately cleaned up",
                key
            );
        });
...
async fn test_delete_large_object_fanout() -> anyhow::Result<()> {
    ...
    assert_eq!(
        volumes_with_file_initial.len(),
        3,
        "All volumes should have large file"
    );
    ...
    assert_eq!(
        delete_status,
        reqwest::StatusCode::NO_CONTENT,
        "DELETE should succeed for large object"
    );
    ...
    assert_eq!(
        meta_after_delete.state,
        TxState::Tombstoned,
        "Meta should be tombstoned for large object"
    );
    ...
    assert_eq!(
        get_status,
        reqwest::StatusCode::NOT_FOUND,
        "GET should return 404 for deleted large object"
    );
...
async fn test_delete_concurrent_operations() -> anyhow::Result<()> {
    ...
    assert!(
        all_success,
        "All concurrent DELETE operations should succeed"
    );
    ...
    for key in &keys {
        ...
        if !volumes_with_file.is_empty() {
            println!(
                "Warning: Key {} still has files on {} volumes",
                key,
                volumes_with_file.len()
            );
        }
