...
impl FaultInjector {
    ...
    pub fn should_fail_pull_mid_stream(&self) -> bool {
        if self.fail_pull_mid_stream_once.load(Ordering::Relaxed) {
            self.fail_pull_mid_stream_once.store(false, Ordering::Relaxed);
            return true;
        }
    ...
    pub fn should_timeout_commit(&self) -> bool {
        if self.fail_commit_timeout_once.load(Ordering::Relaxed) {
            self.fail_commit_timeout_once.store(false, Ordering::Relaxed);
            return true;
        }
    ...
    pub fn reset(&self) {
        ...
        self.fail_etag_mismatch_once.store(false, Ordering::Relaxed);
        self.fail_etag_mismatch_always.store(false, Ordering::Relaxed);
...
pub async fn fail_prepare(
    ...
    if params.once {
        ctx.fault_injector.fail_prepare_once.store(true, Ordering::Relaxed);
    }
    if params.always {
        ctx.fault_injector.fail_prepare_always.store(true, Ordering::Relaxed);
    }
    Ok(StatusCode::OK)
}
...
pub async fn fail_pull(
    ...
    if params.once {
        ctx.fault_injector.fail_pull_once.store(true, Ordering::Relaxed);
        // Also set mid-stream failure for more realistic testing
        ctx.fault_injector.fail_pull_mid_stream_once.store(true, Ordering::Relaxed);
    }
    if params.always {
        ctx.fault_injector.fail_pull_always.store(true, Ordering::Relaxed);
    }
    Ok(StatusCode::OK)
}
...
pub async fn fail_commit(
    ...
    if params.once {
        ...
        if rand::random::<bool>() {
            ctx.fault_injector.fail_commit_once.store(true, Ordering::Relaxed);
        } else {
            ctx.fault_injector.fail_commit_timeout_once.store(true, Ordering::Relaxed);
        }
    }
    if params.always {
        ctx.fault_injector.fail_commit_always.store(true, Ordering::Relaxed);
    }
    Ok(StatusCode::OK)
}
...
pub async fn fail_read_tmp(
    ...
    if params.once {
        ctx.fault_injector.fail_read_tmp_once.store(true, Ordering::Relaxed);
    }
    if params.always {
        ctx.fault_injector.fail_read_tmp_always.store(true, Ordering::Relaxed);
    }
    Ok(StatusCode::OK)
}
...
pub async fn fail_etag_mismatch(
    ...
    if params.once {
        ctx.fault_injector.fail_etag_mismatch_once.store(true, Ordering::Relaxed);
    }
    if params.always {
        ctx.fault_injector.fail_etag_mismatch_always.store(true, Ordering::Relaxed);
    }
    Ok(StatusCode::OK)
}
...
pub async fn inject_latency(
    ...
    if let Some(latency_ms) = params.latency_ms {
        ctx.fault_injector.inject_latency_ms.store(latency_ms, Ordering::Relaxed);
    }
    Ok(StatusCode::OK)
}
...
#[cfg(not(test))]
pub async fn fail_prepare(_: Query<FaultQuery>, _: State<VolumeState>) -> Result<StatusCode, ApiError> {
    Err(ApiError::Any(anyhow::anyhow!("Fault injection only available in test builds")))
}

#[cfg(not(test))]
pub async fn fail_pull(_: Query<FaultQuery>, _: State<VolumeState>) -> Result<StatusCode, ApiError> {
    Err(ApiError::Any(anyhow::anyhow!("Fault injection only available in test builds")))
}

#[cfg(not(test))]
pub async fn fail_commit(_: Query<FaultQuery>, _: State<VolumeState>) -> Result<StatusCode, ApiError> {
    Err(ApiError::Any(anyhow::anyhow!("Fault injection only available in test builds")))
}

#[cfg(not(test))]
pub async fn fail_read_tmp(_: Query<FaultQuery>, _: State<VolumeState>) -> Result<StatusCode, ApiError> {
    Err(ApiError::Any(anyhow::anyhow!("Fault injection only available in test builds")))
}

#[cfg(not(test))]
pub async fn fail_etag_mismatch(_: Query<FaultQuery>, _: State<VolumeState>) -> Result<StatusCode, ApiError> {
    Err(ApiError::Any(anyhow::anyhow!("Fault injection only available in test builds")))
}

#[cfg(not(test))]
pub async fn inject_latency(_: Query<FaultQuery>, _: State<VolumeState>) -> Result<StatusCode, ApiError> {
    Err(ApiError::Any(anyhow::anyhow!("Fault injection only available in test builds")))
}
