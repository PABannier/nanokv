...
async fn test_replicated_write_happy_path_n3() -> anyhow::Result<()> {
    ...
    println!("Starting PUT with {}MB payload to key: {}", payload_size / (1024*1024), key);
    ...
    assert_eq!(status, reqwest::StatusCode::CREATED, "PUT should succeed with 201");
    assert_eq!(etag, expected_etag, "ETag should match blake3 hash");
    assert_eq!(len, expected_len, "Content-Length should match payload size");
    ...
    for expected_replica in &expected_replicas {
        assert!(meta.replicas.contains(expected_replica), 
                "Meta should contain replica: {}", expected_replica);
    }
    ...
    assert_eq!(volumes_with_file.len(), 3, "All 3 volumes should have the file");
    
    // Verify all expected replicas have the file
    for expected_replica in &expected_replicas {
        assert!(volumes_with_file.contains(expected_replica), 
                "Volume {} should have the file", expected_replica);
    }
    ...
    assert!(found_volume, "Location should point to one of our volumes: {}", location_url);
    ...
    assert_eq!(response_bytes, payload, "GET response should match original payload");
    ...
    let meta_after_delete = meta_of(&coord.state.db, key)?.expect("Meta should still exist after delete");
    assert_eq!(meta_after_delete.state, TxState::Tombstoned, "Meta should be tombstoned after delete");
...
async fn test_replicated_write_with_different_key_placements() -> anyhow::Result<()> {
    ...
    for key in &test_keys {
        ...
        let meta = meta_of(&coord.state.db, key)?.expect("Meta should exist");
        assert_eq!(meta.state, TxState::Committed, "Meta should be committed for key: {}", key);
        assert_eq!(meta.replicas.len(), 3, "Should have 3 replicas for key: {}", key);
    }
    ...
    assert!(found_different, "Different keys should have different replica placements");
