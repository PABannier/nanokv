...
use common::constants::{NODE_KEY_PREFIX, META_KEY_PREFIX};
...
fn discover_alive_nodes(db: &KvDb, include_suspect: bool) -> anyhow::Result<Vec<NodeInfo>> {
...
fn build_explicit_volume_urls(volume_urls: Option<Vec<String>>) -> Vec<NodeInfo> {
    let mut nodes = Vec::new();
    match volume_urls {
        None => nodes,
        Some(volume_urls) => {
